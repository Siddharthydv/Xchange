import { fillOrders } from "@/app/utils/httpClient";
...
export function Fills({market}:{market:string}){
    const [data,setData]=useState<Fills[]>([])
    const user=useUser();
    useEffect(()=>{
        async function A(){
            if(user?.user){
            const response= await fillOrders(market);
